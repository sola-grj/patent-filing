...
export async function invalidateErpToken() {
  ...
  if (error) throw new ErpIntegrationError("Unable to invalidate the ERP token.");
}
...
async function erpPost<T>(path: string, body: unknown): Promise<T> {
  ...
  if (!response.ok) {
    throw new ErpIntegrationError(
      `ECI ERP returned HTTP ${response.status}.`,
...
async function getValidToken(forceRefresh = false) {
  ...
  if (!forceRefresh) {
    const { data, error } = await service
      ...
      .eq("provider", PROVIDER)
      .maybeSingle();
    if (error) throw new ErpIntegrationError("Unable to read the ERP token cache.");
    if (
      data
      && tokenIsReusable({
...
async function refreshToken() {
  ...
  if (!response.ok) {
    throw new ErpIntegrationError(
      `ECI ERP authentication returned HTTP ${response.status}.`,
      "erp_auth_failed",
    );
  }
  const payload = await parseJson<Record<string, unknown>>(response);
  const token = tokenFromPayload(payload);
  if (!token) throw new ErpIntegrationError("ECI ERP authentication returned no token.");
  ...
  if (error) throw new ErpIntegrationError("Unable to save the ERP token cache.");
  return token;
}
...
async function parseJson<T>(response: Response): Promise<T> {
  try {
    ...
    throw new ErpIntegrationError("ECI ERP returned an invalid JSON response.");
  }
}
...
function sanitizeRemoteMessage(message?: string) {
  if (!message?.trim()) return "ECI ERP could not complete the request.";
  return message.trim().slice(0, 240).replace(/[\r\n]+/g, " ");
}
