...
export async function getPmRequestDetail(requestId: string) {
  ...
  const [requestResult, approvalResult] = await Promise.all([
    ...
    context.supabase
      ...
      .select("id, subject_id, status, submitted_by, submitted_at, reviewed_by, reviewed_at, decision_reason, sent_at")
  ...
  return {
    ...
    approvalRequests: approvalResult.data ?? [],
  };
}
